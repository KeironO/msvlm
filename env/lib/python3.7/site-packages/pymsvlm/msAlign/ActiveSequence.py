class ActiveSequence:
    ...
    def advance_lower_bound(self):
        ...
        if self.empty():
            raise AttributeError("ActiveSequence::advanceLowerBound(): ActiveSequence must be non empty")
    ...
    def insert(self, heap, peak):
        ...
        if self._the_list == []:
            ...
            if t[0] >= self.num_spectra:
                raise("ActiveSequence::insert(): get<0>(t] >= nbOfSpectra")

            if self._spectra_present[t[0]]:
                raise("ActiveSequence::insert(): this spectra should be absent")
            ...
            new_mz_avg = (float(old_size)*self.mz_avg + mz) / float(old_size +1)
            ...
            if (mz <= new_mz_avg * (1 + self.window_size)):
                if self._the_list[0][2] >= new_mz_avg * (1 - self.window_size):
                    ...
                    self._the_list[heap.pop_and_feed(peak)]
                    return True
